...
## Sku管理
    1.完成SKU模块静态组件以及数据的展示
    2.完成sku的上架与下架操作
        ---v-if与v-else
        ---通过isSale控制，发请求
    3.完成sku查看详情业务
        ---Drawer抽屉、Layout布局、Carousel走马灯(轮播图)、Tag标签
        ---深度选择器
            ---scoped（一般情况下，加上scoped的作用是只是对于当前的组件样式有用）
                ---对于某一个组件，如果style添加上scoped，给当前子组件的结构中都添加上一个 data-v-xxxxxxxx 自定义属性
                ---vue是通过属性选择器，给需要添加的元素添加上样式，如：h3[data-v-7ba5bd90]
            ---子组件的根标签（拥有父组件当中自定义属性：一样的），如果子组件的 根节点 和父组件中书写的标签相同，也会添加上相应父组件的样式
            ---如果父组件的样式（有scoped），而且还想影响到子组件的样式，可以使用深度选择器实现样式穿透
                ---原生CSS   >>>
                ---less      /deep/
                ---scss      ::v-deep
...
## 权限管理
    1.权限管理的介绍
        ...
        ---权限：超级管理员（BOSS），有权利操作整个项目的所有的模块
                产品经理，只能看到首页、商品管理等一部分菜单数据
    2.权限管理业务
        ---用户管理、角色管理、菜单管理
        ---获取数据、展示数据、收集数据
    3.菜单权限
        ---不同的用户（角色），能操作、能查看的菜单是不同的
        ---不同的用户（角色），登录的时候会向服务器发请求，服务器会把用户相应的菜单的权限信息返回给我们
            我们可以根据服务器返回的数据（信息）对比出，动态的设置路由，根据不同的用户展示不同的菜单  
        ---在router.js中整理分类好常量路由、异步路由、任意路由，在vuex中把‘异步路由中的name字段’与‘服务器返回的用户相应菜单的权限信息中routes’对比(注意要递归到children)，把整理出来的‘最终异步路由’与‘常量路由’和‘任意路由’concat在一起，最后router.addRoutes()。
    4.按钮权限
        ---不同的用户（角色），部分按钮，有的用户可见，有的用户不可见
        --- v-show="$store.state.user.buttons.indexOf('btn.Add1')"
